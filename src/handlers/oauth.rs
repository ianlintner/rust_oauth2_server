...
use crate::actors::{AuthActor, CreateAuthorizationCode, TokenActor, CreateToken};
...
pub async fn token(
    ...
    match form.grant_type.as_str() {
        "authorization_code" => handle_authorization_code_grant(form.into_inner(), token_actor, auth_actor).await,
        "client_credentials" => handle_client_credentials_grant(form.into_inner(), token_actor).await,
        ...
        _ => Err(OAuth2Error::unsupported_grant_type(&format!("Grant type '{}' not supported", form.grant_type))),
    }
}
...
async fn handle_authorization_code_grant(
    ...
    let code = req.code.ok_or_else(|| OAuth2Error::invalid_request("Missing code"))?;
    let redirect_uri = req.redirect_uri.ok_or_else(|| OAuth2Error::invalid_request("Missing redirect_uri"))?;
...
async fn handle_client_credentials_grant(
    ...
    let _client_secret = req.client_secret
...
async fn handle_password_grant(
    ...
    let username = req.username.ok_or_else(|| OAuth2Error::invalid_request("Missing username"))?;
    let _password = req.password.ok_or_else(|| OAuth2Error::invalid_request("Missing password"))?;
...
async fn handle_refresh_token_grant(
    ...
    Err(OAuth2Error::unsupported_grant_type("Refresh token grant not yet implemented"))
}
